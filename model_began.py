...
class GeneratorBEGAN(ModelBase):
    ...
    def run(self, bottoms):
        ...
        if self._init:
            print '===== Setup', input_tensor.name.rsplit('/', 1), '====='
...
class DiscriminatorBEGAN(ModelBase):
    def __init__(self, code_dim=128, batchsize=16, input_chn=3, input_size=64, input_depth=4,
            repeat=2, chn=16, kernel_size=3, stride=2, output_chn=3, output_size=64, output_depth=64,
            filler=('msra', 0., 1.), bn=False, alpha=1.0, deconv=False, name='began/dis'):
