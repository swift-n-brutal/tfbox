...
def spec_norm_weight(w, u, niter=1, stop_grad_sigma=False):
    """
        Parameters
        ----------
        w : Tensor
            The tensor of weight with shape [..., out_dim].
        u : Tensor
            The first left (following the conventional term in the origignal
            paper) singular vector with shape [1, out_dim].
        niter : int
            The number of iterations used in the power iteration method.
        stop_grad_sigma : bool
            Whether the singular value, sigma is considered constant or the
            product of v, w and u.
    """
    ...
    for i in xrange(niter-1):
        ...
        u = _l2normalize(vw)
    # stop gradient for the last computation of u and v
    if stop_grad_sigma:
        v = _l2normalize(tf.matmul(u, tf.transpose(w_reshaped)))
        vw = tf.matmul(v, w_reshaped) # v * w
        u = _l2normalize(vw)
        sigma = tf.matmul(vw, u, transpose_b=True)
